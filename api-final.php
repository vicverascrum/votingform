<?php
// API Final compatible con PHP 5.4 y MySQL
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'submit') {
    ...
    if (!isset($input['email']) || !isset($input['totalHours']) || !isset($input['selectedQuestions'])) {
...
} elseif ($_SERVER['REQUEST_METHOD'] === 'GET' && $action === 'test') {
    // Test endpoint
    echo json_encode(array(
        'success' => true,
    ...
    echo json_encode(array('success' => false, 'error' => 'Endpoint no válido. Usa ?action=submit, ?action=results o ?action=test'));
}
?>
